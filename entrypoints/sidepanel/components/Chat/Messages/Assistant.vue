<template>
  ...
  >
    <div class="relative group/message w-full">
      <div
      ...
      <!-- Actions -->
      <div
        v-if="message.done && !disabled && showActions"
        class="flex items-center mt-2 gap-2 opacity-0 group-hover/message:opacity-100 transition-opacity"
      >
        <Tooltip
        ...
        <MessageRetrySelector
          :disabled="disabled"
          @retry="(modelId, endpointType) => emit('retry', modelId, endpointType)"
        />
        <span
          v-if="message.model"
          class="text-text-tertiary text-xs ml-1"
        >
...
import MessageRetrySelector from './MessageRetrySelector.vue'
...
const log = logger.child('chat-assistant-message')
