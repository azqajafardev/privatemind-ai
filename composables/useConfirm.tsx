    ...
    install(app) {
      const confirm = async ({
        ...
        const loading = computed(
          () => confirmLoading.value || cancelLoading.value,
        )
              ...
              <div class="p-8 bg-white rounded-2xl flex flex-col gap-8 w-[520px] max-w-full shadow-[0px_2px_4px_0px_#0000000A,0px_1px_2px_-1px_#00000014,0px_0px_0px_1px_#00000014]">
                ...
                <div class="flex justify-end gap-2 flex-wrap">
                  <Button
                    ...
                    variant="secondary"
                    class={classNames('flex justify-center items-center min-h-8 min-w-36 max-w-full px-2 py-1 font-medium text-xs', btnStyles)}
                    onClick={cancel}
                  >
                    {cancelLoading.value
                      ? (
                      ...
                      : (
                          <span>{cancelText || i18n.t('common.cancel')}</span>
                        )}
                  </Button>
                  <Button
                    disabled={loading.value}
                  ...
                  >
                    {confirmLoading.value
                      ...
                      : (
                          <span>{confirmText || i18n.t('common.confirm')}</span>
                        )}
                  </Button>
                </div>
              </div>
...
export const useConfirm = () => {
  const toast = inject<(options: ConfirmModalOptions) => void>(
    CONFIRM_MODAL_ROOT_PROVIDER,
  )
  return toast!
}
